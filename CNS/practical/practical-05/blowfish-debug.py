...
class Blowfish:
    ...
    def key_expansion(self, key: bytes):      
        ...
        pprint.pprint([hex(x)[2:] for x in self.P])
    ...
    def decrypt(self, data):
        ...
        for i in range(0, len(data), 8):
            ...
            result.extend(left.to_bytes(4, 'big').strip(b'\x00') + right.to_bytes(4, 'big').strip(b'\x00'))
        return bytes(result)
...
cipher=Blowfish(b"\xff"*8, P_ARRAY, S_BOXES)
ciphertext=cipher.encrypt(b"\xff"*8)
...
print(f"{ANSI.GREEN}{ANSI.BOLD}CipherText(HEX):{ANSI.END} {ANSI.GREEN}{ciphertext.hex()}{ANSI.END}")
print(f"{ANSI.BLUE}{ANSI.BOLD}DecipherText:{ANSI.END} {ANSI.BLUE}{deciphertext}{ANSI.END}")
