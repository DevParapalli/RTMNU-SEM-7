...
class AES:
    ...
    def _inv_mix_columns(self, state: list[list[int]]):
        ...
        for c in range(4):
            ...
            state[0][c] = (AES.g_mul_14[temp[0][c]] ^ AES.g_mul_11[temp[1][c]] ^ AES.g_mul_13[temp[2][c]] ^ AES.g_mul_9[temp[3][c]])
            state[1][c] = (AES.g_mul_9[temp[0][c]] ^ AES.g_mul_14[temp[1][c]] ^ AES.g_mul_11[temp[2][c]] ^ AES.g_mul_13[temp[3][c]])
            state[2][c] = (AES.g_mul_13[temp[0][c]] ^ AES.g_mul_9[temp[1][c]] ^ AES.g_mul_14[temp[2][c]] ^ AES.g_mul_11[temp[3][c]])
            state[3][c] = (AES.g_mul_11[temp[0][c]] ^ AES.g_mul_13[temp[1][c]] ^ AES.g_mul_9[temp[2][c]] ^ AES.g_mul_14[temp[3][c]])
        return state
    ...
    def _encrypt(self, plaintext: bytes):
        ...
        state_bytes = 0
        for i in range(4):
                # outer loop moves the row                v
            for j in range(4):
                # inner loop moves the column          v
